    ...
    fn dispatch_core(
        ...
        let result = (|| -> Result<Value, VmError> {
            match capability.kind {
                ...
                HostCapabilityKind::Custom(CapabilityName::StreamBaseWrite) => {
                ...
                HostCapabilityKind::Custom(CapabilityName::ChildSpawnOn) => {
                    ...
                    if matches!(arguments.first(), Some(Value::String(event)) if event == "error") {
                        ...
                        let command = quench_runtime::execute::get_property_result(
                            receiver,
                            "\0childCommand",
                        )
                        ...
                        let error = quench_runtime::host_api::object(vec![
                            ...
                            (
                                "syscall".into(),
                                Value::String(
                                    format!("spawn {}", safe_value_string(&command)).into(),
                                ),
                            ),
                            ("spawnargs".into(), args),
                        ]);
                ...
                HostCapabilityKind::Custom(CapabilityName::CommonSkip) => Ok(Value::Undefined),
                HostCapabilityKind::Custom(CapabilityName::CommonSpawnPromisified) => {
                    Ok(fulfilled(quench_runtime::host_api::object(vec![
                        ("code".into(), Value::Number(1.0)),
                        ("signal".into(), Value::Undefined),
                        ("stdout".into(), Value::String("".into())),
                        ("stderr".into(), Value::String("".into())),
                    ])))
                }
                ...
                HostCapabilityKind::Custom(CapabilityName::CommonExpectsError) => {
                ...
                HostCapabilityKind::Custom(CapabilityName::CommonPlatformTimeout) => {
                    Ok(arguments.first().cloned().unwrap_or(Value::Undefined))
                }
                HostCapabilityKind::Custom(CapabilityName::CommonGetPort) => Ok(Value::Number(0.0)),
                ...
                HostCapabilityKind::Custom(CapabilityName::V8StartupSnapshotThrows) => {
                    Err(thrown_js_error(
                        "Error",
                ...
                HostCapabilityKind::Custom(CapabilityName::V8HeapStats) => {
                    Ok(quench_runtime::host_api::object(vec![
                ...
                HostCapabilityKind::Custom(CapabilityName::V8HeapCodeStats) => {
                    Ok(quench_runtime::host_api::object(vec![
                ...
                HostCapabilityKind::Custom(CapabilityName::FsLstatAsync) => {
                    fs_lstat_async(arguments)
                }
                ...
                HostCapabilityKind::Custom(CapabilityName::FsStatsIsFile) => {
                    Ok(Value::Boolean(false))
                }
                ...
                | HostCapabilityKind::Custom(CapabilityName::FsFdatasyncSync) => {
                    Ok(Value::Undefined)
                }
                ...
                HostCapabilityKind::Custom(CapabilityName::FsOpendirSync) => {
                    self.fs_opendir(arguments)
                }
                ...
                HostCapabilityKind::Custom(CapabilityName::StdioCountZero) => {
                    Ok(Value::Number(0.0))
                }
